    ...
    def init_ask(self):
        ...
        if self.incentive == "A":
            ...
            for i in range(len(self.agent_list)):
                ...
                single_prompt[0]["content"] =  self.agent_list[i].memory[0]["content"]
                single_prompt[1]["content"] =  self.agent_list[i].memory[-1]["content"]
            ...
            for i in range(0, len(batched_prompts), batch_size):
                ...
                # print(len(batch))
                # print(batch[0][0]["content"])
                texts = [self.tokenizer.apply_chat_template(msg, tokenize=False, add_generation_prompt=True) for msg in batch]
                
                model_inputs = self.tokenizer(texts, return_tensors="pt", padding=True, truncation=True).to(device)
